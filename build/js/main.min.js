...
/* harmony import */ var _modules_calendar_init_calendar_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./modules/calendar/init-calendar.js */ "./js/modules/calendar/init-calendar.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('load', function () {
    ...
    window.form = form;
    form.init();
    Object(_modules_calendar_init_calendar_js__WEBPACK_IMPORTED_MODULE_4__["initCalendar"])();
  });
});
  ...
  _createClass(CalendarLayout, [{
    ...
    value: function addSidebar() {
      ...
      var sidebarElement = this._createElement({
        className: 'cld-sidebar'
      });
      var monthListElement = this._createElement({
        tagName: 'ul',
        className: 'cld-monthList'
      });
      var prevElement;
      var nextElement;
      var _loop = function _loop() {
        ...
        var n = i - (4 - _this.calendar.selected.month);
        ...
        if (i === 0) {
          ...
          if (n === 0) {
            var yearElement = _this._createElement({
              tagName: 'li',
              className: 'cld-year'
            });
            if (i < 5) {
              yearElement.innerHTML += _this.calendar.selected.year;
            } else {
              yearElement.innerHTML += _this.calendar.selected.year + 1;
            }
            _this._renderElement(monthListElement, yearElement);
          }
        }
        _this._renderElement(monthListElement, monthElement);
      };
      for (var i = 0; i < this.calendar.monthsLocale.length - 3; i++) {
        var _ret = _loop();
        if (_ret === "continue") continue;
      }
      this._renderElement(sidebarElement, monthListElement);
      if (this.calendar.options.showNavArrows) {
        prevElement = this._createElement({
          className: ['cld-rwd', 'cld-nav']
        });
        nextElement = this._createElement({
          className: ['cld-fwd', 'cld-nav']
        });
      ...
      if (this.calendar.options.navLocation) {
        var locationElement = document.querySelector(this.calendar.options.navLocation);
    ...
    value: function addDateTime() {
      ...
      if (this.calendar.options.datetimeLocation) {
        var locationElement = document.querySelector(this.calendar.options.datetimeLocation);
        locationElement.innerHTML = '';
    ...
    value: function addDays() {
      ...
      for (var i = 0; i < this.calendar.selected.firstDay; i++) {
        dayItemElement = this._createElement({
          tagName: 'li',
          className: ['cld-day', 'prevMonth']
        });

        // Disabled Days
        var disabledDaysCount = i % 7;
        for (var j = 0; j < this.calendar.options.disabledDays.length; j++) {
      ...
      for (var _i = 0; _i < this.calendar.selected.days; _i++) {
        dayItemElement = this._createElement({
          tagName: 'li',
          className: ['cld-day', 'currMonth']
        });

        // Disabled Days
        var _disabledDaysCount = (_i + this.calendar.selected.firstDay) % 7;
        for (var _j = 0; _j < this.calendar.options.disabledDays.length; _j++) {
          if (_disabledDaysCount === this.calendar.options.disabledDays[_j]) {
            dayItemElement.classList.add('disableDay');
          }
        }
      ...
      for (var _i2 = 0; _i2 < extraDaysCount - this.calendar.selected.lastDay; _i2++) {
        dayItemElement = this._createElement({
          tagName: 'li',
          className: ['cld-day', 'nextMonth']
        });

        // Disabled Days
        var _disabledDaysCount2 = (_i2 + this.calendar.selected.lastDay + 1) % 7;
        for (var _j2 = 0; _j2 < this.calendar.options.disabledDays.length; _j2++) {
          if (_disabledDaysCount2 === this.calendar.options.disabledDays[_j2]) {
            dayItemElement.classList.add('disableDay');
          }
        }
    ...
    value: function addEvents() {
      ...
      var currentDays = document.querySelectorAll('.cld-day.currMonth');
      var events;
    ...
    value: function createDayWrap(dayLabel) {
      ...
      var dayWrapElement = this._createElement({
        className: 'cld-day-inner'
      });
      var numberElement = this._createElement({
        className: 'cld-number'
      });
    ...
    value: function getEvents() {
      ...
      fetch(this.element.dataset.eventsUrl).then(function (res) {
        ...
        _this2.calendar.model = data.events;
        _this2.element.dispatchEvent(new CustomEvent('loadDataSuccess'));
        return data;
      });
    }
  }, {
    key: "eventsOnHover",
    value: function eventsOnHover(evt) {
      var element = evt.target.closest('.eventday');
      if (!element) {
        return;
      }
      var linksWrap = element.querySelector('.cld-links');
      var wrapBoxCrd = linksWrap.getBoundingClientRect();
      var vw = document.documentElement.clientWidth;
      if (wrapBoxCrd.width >= vw) {
        // смещение тултипа с событиями если он не помещается в окна просмотра
        linksWrap.style.width = "".concat(vw - this.calendar.options.tooltipOffsetEdge * 2, "px");
        wrapBoxCrd = linksWrap.getBoundingClientRect();
        linksWrap.style.left = "".concat(vw - (wrapBoxCrd.x + wrapBoxCrd.width) - this.calendar.options.tooltipOffsetEdge, "px");
    ...
    key: "eventsOnHoverBlur",
    value: function eventsOnHoverBlur(evt) {
      var element = evt.target.closest('.eventday');
      if (!element) {
        return;
      }
    ...
    value: function _createElement(props) {
      ...
      var element = document.createElement(props.tagName || 'div');
...
/***/ "./js/modules/calendar/init-calendar.js":
/*!**********************************************!*\
  !*** ./js/modules/calendar/init-calendar.js ***!
  \**********************************************/
...
/* harmony import */ var _calendar_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./calendar.js */ "./js/modules/calendar/calendar.js");

var initCalendar = function initCalendar() {
  var calendarElement = document.querySelector('.event-cldr');
  if (!calendarElement) {
    return;
  }
  var options = {
  ...
  var calendar = new _calendar_js__WEBPACK_IMPORTED_MODULE_0__["EventCalendar"](calendarElement, options);
};
